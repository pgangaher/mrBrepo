...
def returns(close: pd.Series, periods=(21, 63, 126)) -> dict:
    out = {}
    for p in periods:
        if len(close) > p:
...
def relative_strength(close: pd.Series, bench_close: pd.Series, period: int = 63):
...
def beta(close: pd.Series, bench_close: pd.Series, period: int = 63):
    if len(close) < period + 1 or len(bench_close) < period + 1:
        return None
...
def n_day_high(high: pd.Series, n: int):
    if len(high) < n:
        return None
    return float(high.iloc[-n:].max())


def n_day_low(low: pd.Series, n: int):
    if len(low) < n:
        return None
...
def sma(close: pd.Series, period: int):
    if len(close) < period:
        return None
